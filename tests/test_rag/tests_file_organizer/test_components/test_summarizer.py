...
from rag.file_organizer.src.config.config import SummarizerCfg
from rag.file_organizer.src.core.summarizer import Summarizer
from rag.file_organizer.src.services.models import MockModel, NvidiaModel
...
class TestSummarizer:
    ...
    @pytest.mark.asyncio
    async def test_generate_summary_with_mock_model(self, config, mock_model, sample_markdown_file):
        summarizer = Summarizer(config, mock_model)
    ...
    @pytest.mark.asyncio
    async def test_process_folder(self, config, mock_model, temp_dir, sample_markdown_file):
        summarizer = Summarizer(config, mock_model)
    ...
    @pytest.mark.asyncio
    async def test_batch_generate_summaries(self, config, mock_model, sample_markdown_file):
        summarizer = Summarizer(config, mock_model)
    ...
    @pytest.mark.skipif(not os.getenv("NVIDIA_API_KEY"), reason="Local HF model tests disabled")
    @pytest.mark.asyncio
