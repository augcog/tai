...
class DatabaseInitializer:
    ...
    def initialize_database(self) -> bool:
        ...
        try:
            ...
            if self.data_dir.exists():
                ...
                logger.info(f"📁 Data directory {self.data_dir} does not exist, skipping file import")
                ...
                try:
                    for row in existing_data:
                        file_record = FileRegistry(
                            ...
                            size_bytes=row[4],
                            course_code=row[5],
                            category=row[6] if row[6] in ['document', 'video', 'audio', 'other'] else 'other',
                            title=row[7],
                    ...
                    session.commit()
                    logger.info(f"✅ Migrated {len(existing_data)} files to modern schema")
                except Exception as e:
                    session.rollback()
        ...
        try:
            ...
            for file_path in self._scan_data_directory():
                ...
                try:
                    ...
                    try:
                        relative_path = str(abs_file_path.relative_to(self.data_dir.resolve()))
    ...
    def _update_course_registry(self) -> int:
        ...
        try:
            ...
            for course in session.query(CourseModel).all():
                existing_courses.add(course.course_code)
    ...
    def get_database_status(self) -> Dict:
        ...
        try:
            ...
            file_count = session.query(FileRegistry).filter(FileRegistry.is_active == True).count()
