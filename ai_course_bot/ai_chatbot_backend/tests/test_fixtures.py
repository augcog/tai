...
class TestFixtureStructure:
    ...
    def test_request_fixtures_structure(self, fixtures):
        ...
        for fixture in fixtures["requests"]:
            ...
            assert "description" in meta, f"Missing 'description' in meta section of {path}"
            ...
            # Check data for request fixtures
            data = fixture["data"]
            if "rag" in meta.get("tags", []):
                assert "rag" in data, f"Missing 'rag' flag in data section of {path}"
            
            if "streaming" in meta.get("tags", []):
                assert "stream" in data, f"Missing 'stream' flag in data section of {path}"
            
            # If it's an OpenAI format request, check required fields
            if "openai-format" in meta.get("tags", []):
                assert "messages" in data, f"Missing 'messages' in data section of {path}"
                assert isinstance(data["messages"], list), f"'messages' should be a list in {path}"
                assert len(data["messages"]) > 0, f"'messages' should not be empty in {path}"
                ...
                for message in data["messages"]:
                    ...
                    assert message["role"] in ["system", "user", "assistant"], f"Invalid role '{message['role']}' in {path}"
    ...
    def test_response_fixtures_structure(self, fixtures):
        ...
        for fixture in fixtures["responses"]:
            ...
            if "streaming" in meta.get("tags", []):
                assert isinstance(data, list), f"Data should be a list for streaming response in {path}"
                for chunk in data:
                ...
                assert isinstance(data, dict), f"Data should be a dict for non-streaming response in {path}"
                ...
                for choice in data["choices"]:
                    ...
                    assert "finish_reason" in choice, f"Missing 'finish_reason' in choice in {path}"
            
            # Check for tool_calls or references if they are part of the tags
            if "references" in meta.get("tags", []) or "tool-calls" in meta.get("tags", []):
                if "streaming" in meta.get("tags", []):
                        ...
                        for choice in chunk["choices"]:
                            if "delta" in choice and "tool_calls" in choice["delta"]:
                                ...
                                for tool_call in choice["delta"]["tool_calls"]:
                                    assert "type" in tool_call, f"Missing 'type' in tool_call in {path}"
                                    assert "function" in tool_call, f"Missing 'function' in tool_call in {path}"
                    ...
                    for choice in data["choices"]:
                        assert "message" in choice, f"Missing 'message' in choice in {path}"
                        assert "tool_calls" in choice["message"], f"Missing 'tool_calls' in message in {path}"
    ...
    def test_validation_sections(self, fixtures):
        ...
        for fixture_type in ["requests", "responses"]:
            for fixture in fixtures[fixture_type]:
                path = fixture.get("_path", "unknown")
                ...
                if "validation" in fixture:
                    ...
                    if "expected_status_code" in validation:
                        assert isinstance(validation["expected_status_code"], int), f"'expected_status_code' should be an integer in {path}"
                    
                    if "content_type" in validation:
                        assert isinstance(validation["content_type"], str), f"'content_type' should be a string in {path}"
                    
                    # Response-specific validation
                    if fixture_type == "responses" and "streaming" in fixture.get("meta", {}).get("tags", []):
                        if "expected_content_chunks" in validation:
                            assert isinstance(validation["expected_content_chunks"], int), f"'expected_content_chunks' should be an integer in {path}"
                        
                        if "expected_tool_call_chunks" in validation:
                            assert isinstance(validation["expected_tool_call_chunks"], int), f"'expected_tool_call_chunks' should be an integer in {path}"
