...
class FuncClassifier:
    ...
    def _get_predefined_function(self, file_path: str, predefined_classifications) -> str:
    ...
    async def process_summaries(self, summaries_path: str, output_path: str, predefined_classification: Dict = None) -> Dict:
        ...
        for file_path, summary in summaries.items():
            if predefined_classification:
                predefined_func = self._get_predefined_function(file_path, predefined_classification)
                if predefined_func:
        ...
        chunk_size = 10
        chunks = [list(rest_summaries.items())[i:i + chunk_size] for i in range(0, len(rest_summaries), chunk_size)]
        for chunk in chunks:
