...
from app.core.db_initializer import DatabaseInitializer, get_initializer
...
Examples:
  ...
  %(prog)s --data-dir /path/to/data # Use custom data directory
    ...
    if args.check:
        ...
        print(f"  Data directory exists: {'✅ Yes' if status['data_directory_exists'] else '❌ No'}")
        
        if status['data_directory_exists']:
            # Count files in data directory
            data_path = Path(args.data_dir)
            file_count = len([f for f in data_path.rglob('*') if f.is_file() and not f.name.startswith('.')])
