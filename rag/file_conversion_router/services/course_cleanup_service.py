...
def cleanup_course_cli(database_path: str, course_code: Optional[str] = None,
                       course_name: Optional[str] = None, dry_run: bool = False):
    """
    Command-line interface for course cleanup.
    ...
    """
    service = CourseCleanupService(database_path)
    
    # Show current statistics
    print("\n=== Current Course Statistics ===")
    try:
        stats = service.get_course_statistics(course_code, course_name)
    ...
    # Perform cleanup
    print(f"\n=== {'Dry Run' if dry_run else 'Cleanup'} ===")
    result = service.cleanup_course(course_code, course_name, dry_run)
    
    print(f"Status: {result['message']}")
    if 'files_deleted_count' in result:
        print(f"Files deleted: {result['files_deleted_count']}")
        print(f"Orphaned chunks deleted: {result.get('orphaned_chunks_deleted', 0)}")
    
    # Vacuum if actual deletion occurred
    if not dry_run and result.get('success') and result.get('files_deleted_count', 0) > 0:
        print("\n=== Vacuuming Database ===")
        vacuum_result = service.vacuum_database()
        print(f"Space saved: {vacuum_result['space_saved_mb']} MB")


if __name__ == "__main__":
    import argparse
    
    parser = argparse.ArgumentParser(description="Clean up course data from RAG database")
    parser.add_argument("database", help="Path to the SQLite database")
    parser.add_argument("--course-code", help="Course code to delete (e.g., 'CS 61A')")
    parser.add_argument("--course-name", help="Course name to delete")
    parser.add_argument("--dry-run", action="store_true", help="Simulate deletion without making changes")
    parser.add_argument("--list-courses", action="store_true", help="List all courses in the database")
    parser.add_argument("--info", action="store_true", help="Show database information")
    
    args = parser.parse_args()
    
    service = CourseCleanupService(args.database)
    ...
    if args.list_courses:
        ...
        if info['top_courses']:
            ...
            for course in info['top_courses']:
                print(f"  {course['course_code']}: {course['course_name']} ({course['file_count']} files)")
    else:
        if not args.course_code and not args.course_name:
            print("Error: Either --course-code or --course-name must be provided")
            parser.print_help()
        else:
            cleanup_course_cli(args.database, args.course_code, args.course_name, args.dry_run)