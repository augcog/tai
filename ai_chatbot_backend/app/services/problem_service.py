...
from app.core.models.content import ContentProblemModel
...
class ProblemService:
    ...
    @staticmethod
    def get_problems_by_file_uuid_v2(db: Session, file_uuid: str) -> List[ContentProblemModel]:
        """Get all problems for a specific file"""
        try:
            return db.query(ContentProblemModel).filter(
                ContentProblemModel.file_uuid == file_uuid
            ).all()
        except SQLAlchemyError as e:
            logger.error(f"Database error retrieving problems for file {file_uuid}: {str(e)}")
            return []
