...
def test_embedding_optimizer_integration_with_local_llama_3_2_3b(sample_chunks):
    ...
    assert optimized_chunks[0].content != copy_old_chunk_content[0], (
    ...
    assert "transfer learning" in optimized_chunks[1].content, (
        "Content should contain 'TAI'."
    )
    assert optimized_chunks[1].content != copy_old_chunk_content[1], (
        "Content should be different."
    )
...
def test_server_model_generate_summary_integration(
    ...
    try:
        ...
        for idx, chunk in enumerate(processed_chunks):
            ...
            assert chunk.content != sample_chunks[idx].content, (
