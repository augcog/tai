...
def bge_compute_score(
    ...
    for i in range(len(document_embeddings)):
        dense_score = (
            query_embedding["dense_vecs"] @ document_embeddings[i]["dense_vecs"].T
        )
...
for n in [400]:
    ...
    if technique == "recursive_seperate":
        ...
        with open(
            f"pickle/{technique}_{method}_BGE_embedding_{n}_106_full.pkl", "rb"
        ) as f:
    ...
    for i, (id, question) in enumerate(questions):
        ...
        if (
            method == "none"
            or method == "sum"
            or method == "connected_bullet"
            or method == "to_task_chat_completion"
        ):
            ...
            history = [
                ...
                {"role": "user", "content": chat_completion(system_prompt, question)},
            ]
        else:
            history = [
                {"role": "system", "content": system_query_prompt},
        ...
        if model == "local":
            query_embed = np.array(
                openai.Embedding.create(
                    model="text-embedding-ada-002", input=wizard_coder(history)
                )["data"][0]["embedding"]
            )
        ...
        cosine_similarities = np.array(
            bge_compute_score(query_embed, embedding_list, [1, 1, 1], None, None)[
                "colbert+sparse+dense"
            ]
        )
        ...
        data_to_pickle = {"question": question, "doc": doc, "doc_id": doc_id}
        ...
        ids_without_number = [
            remove_number(id)
            for id in ids
            if not (remove_number(id) in seen or seen.add(remove_number(id)))
        ][:10]
