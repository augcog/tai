...
class OpenAIModel(LLMBase):
    def __init__(self, model_name: str, api_key=None, max_new_tokens=1024,
            temperature: float = 0.7,):
        self.api_key = api_key or os.getenv("OPENAI_API_KEY") or ""
        if not self.api_key:
            raise ValueError("OpenAI API key must be provided either as an argument or via environment variable.")
...
class NvidiaModel(LLMBase):
    ...
    def __init__(
        ...
        if not self.api_key:
            raise ValueError("NVIDIA API key must be provided either as an argument or via environment variable.")
