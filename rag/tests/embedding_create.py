...
if method == "to_task":
    ...
    system_query_prompt = (
        "Given the task, generate a document that can help you to answer this task."
    )
elif method == "sum":
...
for n in [400]:
    ...
    for doc in tqdm(docs, desc="Processing documents"):
        ...
        for chunk in file:
            ...
            if technique == "seperate_paragraph":
                ...
                segment = [
                    constraint[2:] if constraint.startswith("- ") else constraint
            ...
            for smaller_chunk in segment:
                hp = human_embedding_prompt.format(
                    segment=smaller_chunk,
                    segment_path=folder_path + " > " + segment_path,
                )
                try:
                    ...
                    if model == "local":
                        ...
                        embedding.append(
                            openai.Embedding.create(
                                model="text-embedding-ada-002", input=hp.strip()
                            )["data"][0]["embedding"]
                        )
                    elif model == "zephyr":
                        embedding.append(
                            openai.Embedding.create(
                                model="text-embedding-ada-002", input=gpt(history)
                            )["data"][0]["embedding"]
                        )
                        ...
                        embedding.append(
                            openai.Embedding.create(
                                model="text-embedding-3-small", input=gpt(history)
                            )["data"][0]["embedding"]
                        )
                    elif model == "openai_3_large":
                        embedding.append(
                            openai.Embedding.create(
                                model="text-embedding-3-large", input=gpt(history)
                            )["data"][0]["embedding"]
                        )
                    elif model == "cohere":
                        embedding.extend(
                        ...
                        embed = last_token_pool(
                            output.last_hidden_state, batch_dict["attention_mask"]
                        )
    ...
    if technique == "recursive_seperate":
        with open(
            f"{folder_name}/{technique}_{method}_{model}_embedding_{n}_106_full.pkl",
            "wb",
        ) as f:
            pickle.dump(data_to_store, f)
    else:
        with open(
            f"{folder_name}/{technique}_{method}_{model}_embedding.pkl", "wb"
        ) as f:
            pickle.dump(data_to_store, f)
