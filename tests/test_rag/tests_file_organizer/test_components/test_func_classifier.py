...
from rag.file_organizer.src.core.file_organizer import ContentCategory
...
class TestFuncClassifier:
    ...
    @pytest.mark.asyncio
    async def test_process_summaries_with_predefined_classification(self, config, mock_model, summaries_file, temp_dir):
        classifier = FuncClassifier(config, mock_model)
        ...
        result = await classifier.process_summaries(str(summaries_file), str(output_path), predefined_classification)
    ...
    @pytest.mark.asyncio
    async def test_process_summaries_with_mock_model(self, config, mock_model, summaries_file, temp_dir):
        classifier = FuncClassifier(config, mock_model)
        output_path = temp_dir / "functions.json"

        result = await classifier.process_summaries(str(summaries_file), str(output_path))
    ...
    async def test_batch_classify_with_openai_model(self, config, temp_dir):
        ...
        summaries = {
            str(temp_dir / "physics_problem.md"): "A set of physics problems regarding kinematics.",
            str(temp_dir / "history_notes.md"): "Lecture notes on the French Revolution."
        }
    ...
    @pytest.mark.skipif(not os.getenv("OPENAI_API_KEY"), reason="OpenAI API key not set")
    @pytest.mark.asyncio
    async def test_process_summaries_with_openai_model(self, config, summaries_file, temp_dir):
        model = OpenAIModel("gpt-4.1")
        classifier = FuncClassifier(config, model)
