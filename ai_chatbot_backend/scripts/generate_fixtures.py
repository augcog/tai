...
TEMPLATES = {
    "base_meta": {"name": "", "description": "", "tags": []},
    "base_request": {
        "model": "custom-model",
        "messages": [],
        "temperature": 0.7,
    ...
    "base_validation": {"expected_status_code": 200},
}
...
MESSAGES = {
    ...
    "user_cs61a": {"role": "user", "content": "What topics are covered in CS61A?"},
}
...
def create_non_streaming_response(
    with_references: bool = False, course: str = "TAI"
) -> Dict:
    ...
    content = (
        CONTENT["cs61a_description"]
        if course == "CS61A"
        else CONTENT["tai_description"]
    )
...
def generate_all_fixtures():
    ...
    rag_request_meta = create_meta(
        ...
        validation={"expected_status_code": 200, "content_type": "application/json"},
    )
    create_request_fixture(
        "rag_request.json", rag_request_meta, create_rag_request(streaming=False)
    )
    ...
    streaming_rag_request_meta = create_meta(
        ...
        validation={"expected_status_code": 200, "content_type": "text/event-stream"},
    )
    create_request_fixture(
