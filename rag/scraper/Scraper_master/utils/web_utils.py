...
def join_url(root_url: str, href: str) -> str:
    ...
    if not root_parsed.scheme or not root_parsed.netloc:
        raise ValueError(
            "root_url must be a valid absolute URL (with scheme and netloc)."
        )
...
def get_file_name(url):
    ...
    if len(filename) > MAX_FILENAME_LENGTH:
        truncated_name = name[
            : MAX_FILENAME_LENGTH - len(ext)
        ]  # Adjust for extension length
...
if __name__ == "__main__":
    print(
        normalize_url(
            "https://www.youtube.com/watch?v=IPec2A7j2bY&amp;list=PL6BsET-8jgYVCz97Y75GRXSWbb4sTpDIR"
        )
    )
