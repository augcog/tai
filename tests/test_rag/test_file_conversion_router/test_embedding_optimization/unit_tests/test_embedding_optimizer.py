...
def create_config_file(tmp_path, tasks, model_name='mock-model', model_type='test_mock',
...
class TestEmbeddingOptimizer:
    ...
    def test_error_handling(self, config_path_for_test, mock_server_model):
        ...
        with patch('rag.file_conversion_router.embedding_optimization.src.pipeline.builder.ServerModelTAI',
            ...
            assert processed_chunks[0].metadata['error_message'] == 'Empty content is not allowed'
    ...
    def test_metadata_preservation(self, config_path_for_test, mock_server_model, sample_chunks):
        ...
        with patch('rag.file_conversion_router.embedding_optimization.src.pipeline.builder.ServerModelTAI',
            ...
            for original, processed in zip(sample_chunks, processed_chunks):
                ...
                assert processed.metadata.get('original_length') == original.metadata['original_length']
                assert 'task_results' in processed.metadata

    def test_basic_processing(self, config_path_for_test, mock_server_model, sample_chunks):
